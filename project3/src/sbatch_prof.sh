#!/bin/bash
#SBATCH -o ./Project3-results_prof.txt
#SBATCH -p Release
#SBATCH -J Project3
#SBATCH --nodes=1
...
do
  echo "Number of cores: $num_cores"
  srun -n 1 --cpus-per-task $num_cores perf stat -e cpu-cycles,cache-misses,page-faults ../build/src/mergesort/mergesort $num_cores $DATA_SIZE
done
echo ""
...
srun -n 1 --gres=gpu:1 nsys profile -t cuda,nvtx,osrt -o ./radixsort.qdrep ../build/src/radixsort-gpu/radixsort $DATA_SIZE
echo ""
...
do
  echo "Number of cores: $num_cores"
  srun -n 1 --cpus-per-task $num_cores perf stat -e cpu-cycles,cache-misses,page-faults ../build/src/searching-cpu/searching_array_parallel $num_cores $SEARCH_SIZE
done

echo "Parallel Searching for Data Array on GPU (Optimized with -O2)"
srun -n 1 --gres=gpu:1 nsys profile -t cuda,nvtx,osrt -o ./search.qdrep ../build/src/searching-gpu/searching_array_gpu $SEARCH_SIZE
